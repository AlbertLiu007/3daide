...
export function calculateQuote({ measurement, material, quantity }: QuoteInput): QuoteResult {
  ...
  const supportCost = (materialCost + surfaceAreaCost) * material.supportRate;
  const failureBuffer = (materialCost + surfaceAreaCost) * material.failureRate;
  const postProcessCost = material.postProcessFee;
  const subtotalCost = materialCost + surfaceAreaCost + supportCost + failureBuffer + postProcessCost;
